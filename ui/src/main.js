...
function updateServiceUI() {
    ...
    if (isServiceRunning) {
        indicator.className =
            "w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.5)] transition-colors";
        label.innerText = "Running";
        label.className = "text-emerald-400";
        btn.innerText = "Stop";
        btn.className =
            "text-xs px-3 py-1.5 bg-red-600/80 hover:bg-red-500 text-white rounded transition-colors font-medium cursor-pointer";
    } else {
        indicator.className =
            "w-2.5 h-2.5 rounded-full bg-slate-500 transition-colors";
        label.innerText = "Stopped";
        label.className = "text-slate-400";
        btn.innerText = "Start";
        btn.className =
            "text-xs px-3 py-1.5 bg-blue-600/90 hover:bg-blue-500 text-white rounded transition-colors font-medium cursor-pointer";
    }
}
...
async function saveConfig() {
    try {
        ...
        if (running) {
            showConfirm(
                "Configuration saved! The background service is currently running. Would you like to restart it to apply the new settings?",
                async () => {
                    try {
...
function updateAuthUI(state) {
    ...
    if (state.authenticated) {
        if (badge) {
            ...
            badge.className =
                "text-[10px] px-2 py-0.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 text-emerald-400";
        }
        ...
        if (badge) {
            ...
            badge.className =
                "text-[10px] px-2 py-0.5 rounded-full border border-slate-700 text-slate-500";
        }
        if (msg)
            msg.innerText =
                state.error || "You must authenticate to use TeleFeed.";
...
$("#btn-auth-2fa")?.addEventListener("click", async () => {
    ...
    btn.innerText = "...";
    try {
        const res = await api("/api/auth/2fa", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
...
function renderAreas() {
    ...
    areas.forEach((area, idx) => {
        const el = document.createElement("div");
        el.className =
            "glass-panel p-5 relative group hover:border-slate-600 transition-colors";

        const kwHtml = (area.keywords || [])
            .map((k) => `<span class="badge badge-blue">${esc(k)}</span>`)
            .join("");
        const negKwHtml = (area.negative_keywords || [])
            .map((k) => `<span class="badge badge-red">${esc(k)}</span>`)
            .join("");
        const srcHtml = (area.sources || [])
            .map((s) => `<span class="badge badge-gray">${esc(s)}</span>`)
            .join("");
    ...
    // Bind edit/delete
    $$(".btn-area-edit").forEach((btn) =>
        btn.addEventListener("click", () =>
            openAreaModal(parseInt(btn.dataset.idx, 10)),
        ),
    );
    $$(".btn-area-del").forEach((btn) =>
        btn.addEventListener("click", () => {
            const idx = parseInt(btn.dataset.idx, 10);
            showConfirm(
                `Delete area "${globalConfig.areas[idx].name}"?`,
                () => {
...
function renderMarkdown(text) {
    ...
    html = html.replace(
        /```(?:[a-zA-Z0-9_-]+)?\n?([\s\S]*?)```/g,
        (match, code) => {
            return `<pre class="bg-slate-900/90 p-3 rounded-lg border border-slate-800/80 overflow-x-auto text-xs font-mono my-2 text-emerald-400"><code>${code.trim()}</code></pre>`;
        },
    );

    html = html.replace(
        /`([^`]+)`/g,
        '<code class="bg-slate-800/80 px-1.5 py-0.5 rounded text-xs text-emerald-300 font-mono">$1</code>',
    );
    html = html.replace(
        /(\*\*|__)(.*?)\1/g,
    ...
    html = html.replace(
        /\[([^\]]+)\]\(([^)]+)\)/g,
        '<a href="$2" target="_blank" rel="noopener" class="text-blue-400 hover:text-blue-300 underline">$1</a>',
    );
    html = html.replace(/\n/g, "<br />");
...
function renderMatchCard(match, prepend = true) {
    ...
    const el = document.createElement("div");
    el.className =
        "glass-panel p-4 transition-all duration-300 hover:border-slate-600";
    ...
    const timeStr = match.matched_at
  ...
  `;
    ...
    el.querySelectorAll(".btn-match-status").forEach((b) =>
        b.addEventListener("click", async () => {
            try {
                await api(
                    `/api/matches/${b.dataset.id}/status?new_status=${b.dataset.status}`,
                    { method: "POST" },
                );
...
$("#btn-svc-install")?.addEventListener("click", () =>
    showConfirm("Install the TeleFeed background service?", () =>
        actionService("install"),
    ),
);
$("#btn-svc-uninstall")?.addEventListener("click", () =>
    showConfirm("Uninstall the TeleFeed background service?", () =>
        actionService("uninstall"),
    ),
);
$("#btn-svc-restart")?.addEventListener("click", () =>
...
async function loadLogs() {
    ...
    try {
        ...
        container.innerHTML =
            '<span class="text-red-400">Failed to load logs.</span>';
    }
}
...
$("#btn-run-doctor")?.addEventListener("click", async () => {
    ...
    container.innerHTML =
        '<span class="text-slate-500 italic text-sm">Running diagnostics...</span>';
    try {
      ...
      <div class="flex items-center gap-2 text-sm py-1.5 px-2 rounded ${c.ok ? "" : "bg-red-500/5"}">
        ...
        <span class="font-medium w-40 shrink-0">${esc(c.label)}</span>
    ...
    `,
        ...
        container.innerHTML =
            '<span class="text-red-400 text-sm">Diagnostics failed.</span>';
    }
});
